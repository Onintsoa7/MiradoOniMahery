...
class oni extends CI_Model
{
    ...
    {
        if ($id == 0) {
            $query = "SELECT SUM(regime.prix) as prix,SUM(poidsvariation) as poidsvariation,regime_alimentaire_relation.idregime
    ...
    //1 = utilisable
    //11 = non utilisable
    //0 = en cours de demande
    public function getcode($etat)
    {
        if ($etat == 0) {
            $query = "select * from code where etat = 0";
        } elseif ($etat == 1) {
